...
type ApiOk = { pm?: unknown; odoo?: unknown; };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const group of chunk(ids, 120)) {
        try {
          const res = await fetch(
            `/api/comments/am?ids=${encodeURIComponent(group.join(","))}`,
            { cache: "no-store" }
          );
      ...
      for (const group of chunk(ids, 120)) {
        try {
          const res = await fetch(
            `/api/comments/pm?ids=${encodeURIComponent(group.join(","))}`,
            { cache: "no-store" }
          );
          const txt = await res.text(); if (!txt) continue;
          const json = safeJson<{ items: Array<{ id: string; comment: string | null }> }>(txt);
  ...
  return (
    <main className="white-root">
      ...
      <style jsx>{`
        ...
        .legendBar { display:flex; align-items:center; justify-content:space-between; gap:12px; width:100%; max-width:1100px; margin-top:6px; }
        .subtotal { font-size:13px; color:#374151; }
        .legend { display:flex; gap:12px; font-size:13px; color:#374151; justify-content:flex-end; flex-wrap:wrap; }
        .legendItem { display:inline-flex; align-items:center; gap:6px; padding:3px 10px; border-radius:6px; background:#f3f4f6; }
